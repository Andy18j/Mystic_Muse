...
async function getData() {
    try {
        const response = await fetch("https://mystic-muse-backend.onrender.com/product/");
        const data = await response.json();
        const coatData = data.getdata; // Accessing the array under 'getdata' key
        if (Array.isArray(coatData)) {
            globalData = coatData;
            renderData(globalData);
            showPagination(globalData.length, itemsPerPage); // Call showPagination after fetching data
        } else {
...
function addProductEventListeners() {
    ...
    products.forEach(product => {
        product.addEventListener("click", () => {
            const itemId = product.getAttribute("data-id");
            const selectedItem = globalData.find(item => item.id === itemId);
            if (selectedItem) {
                localStorage.setItem("product-id", selectedItem.id);
                window.open("product-desc.html", "_blank"); // Open in a new tab
            }
        });
    });
}

// Call addProductEventListeners after rendering data
function renderData(data) {
    const productEl = document.getElementById("product-container");
    productEl.innerHTML = data.map(item => getProductHTML(item)).join("");
